...
const httpRequest = async (input) => {
    ...
    if (!response.ok) {
        throw new Error(`http:request failed – status ${response.status} ${response.statusText} from ${url} – body: ${JSON.stringify(responseBody)}`);
    }
    return {
        outputPayload: {
